...
export function NFTDetailPage({ onQuickBuy }: NFTDetailPageProps) {
  ...
  const relatedNFTs = mockNFTs.filter(n => n.id !== id && n.category === nft?.category).slice(0, 4);
                  ...
                  <div className="space-y-4">
                    {nft.history.map((event, index) => (
                      <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-4">
                          <Badge variant={
                            event.event === 'Minted' ? 'default' :
                            event.event === 'Sold' ? 'destructive' : 'secondary'
                          }>
                            {event.event}
                          </Badge>
                          <div>
                            ...
                            {event.to && (
                              <p className="text-sm">
                                To <span className="font-medium">{event.to}</span>
                              </p>
                            )}
                          </div>
                        </div>
                        <div className="text-right">
                          {event.price && (
                            <p className="font-medium">{event.price} ETH</p>
                          )}
